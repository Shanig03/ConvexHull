...
// Proactor test callback: echoes received data and closes the connection
void* proactorClientHandler(int clientfd) {
    char buffer[256];
    int bytesRead = read(clientfd, buffer, sizeof(buffer) - 1);
    if (bytesRead > 0) {
        buffer[bytesRead] = '\0';
        std::cout << "[Proactor] Received: " << buffer << std::endl;
...
void testProactor() {
    ...
    addr.sin_family = AF_INET;
    addr.sin_addr.s_addr = INADDR_ANY;
    addr.sin_port = htons(12346); // Use a different port from reactor test

    assert(bind(serverSocket, (struct sockaddr*)&addr, sizeof(addr)) == 0);
    assert(listen(serverSocket, 5) == 0);

    // Start proactor
    pthread_t proactor_tid = startProactor(serverSocket, proactorClientHandler);
    assert(proactor_tid != 0);

    // Give the proactor a moment to start
    ...
    assert(connect(clientSocket, (struct sockaddr*)&serverAddr, sizeof(serverAddr)) == 0);
    ...
    assert(bytesRead > 0);
    buffer[bytesRead] = '\0';
    std::cout << "[Proactor] Echoed: " << buffer << std::endl;
    ...
    std::cout << "✓ Proactor test completed" << std::endl;
}
